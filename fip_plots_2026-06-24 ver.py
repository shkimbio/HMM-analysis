...
_spec = _ilu.spec_from_file_location(
    "fip_pipeline",
    _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "fip_pipeline_2026-06-24 ver.py"))
fp = _ilu.module_from_spec(_spec); _spec.loader.exec_module(fp)
...
def main():
    ...
    summary_states_threat, summary_states_shelter = [], []
    ...
    for m in cfg["mice"]:
        ...
        for stem, store, state_store in (
                ("threat",  summary_threat,  summary_states_threat),
                ("shelter", summary_shelter, summary_states_shelter)):
            dfp = os.path.join(td, f"peri_{stem}_dff_{mid}.csv")
            psp = os.path.join(td, f"peri_{stem}_states_{mid}.csv")
            ...
            if os.path.exists(psp) and os.path.getsize(psp) > 5:
                P = pd.read_csv(psp)
                if len(P):
                    state_store.append((f"{mid} {m['sensor']}", P, primary))
    ...
    # ---- NEW SUMMARY figs: per-sensor ΔF/F + HMM P(state) separated by row ----
    for stem, dff_store, state_store, evlab, dur in (
            ("threat",  summary_threat,  summary_states_threat,  "Threat onset",  cfg["events"]["threat_dur_s"]),
            ("shelter", summary_shelter, summary_states_shelter, "Shelter entry", None)):
        if not dff_store or not state_store:
            continue
        evt_col = cfg["figure_style"]["event_palette"]["Threat" if stem == "threat" else "Shelter Return"]
        n_sensors = len(dff_store)
        fig, axes = plt.subplots(n_sensors * 2, 1, figsize=(8.5, 3.5 * n_sensors),
                                 sharex=True, gridspec_kw={"hspace": 0.08})
        for i, ((name, tt, z, prim), (_, P, _)) in enumerate(zip(dff_store, state_store)):
            ax_dff = axes[i * 2]
            ax_st  = axes[i * 2 + 1]

            # ΔF/F panel
            ax_dff.plot(tt, z, lw=2.0, color=cpal.get(prim, "#555"))
            ax_dff.axvline(0, color=evt_col, lw=1.4, ls="--")
            if dur:
                ax_dff.axvspan(0, dur, color=evt_col, alpha=0.07, lw=0)
            ax_dff.set_ylabel("z-scored ΔF/F", fontsize=9)
            ax_dff.text(0.01, 0.92, name, transform=ax_dff.transAxes,
                        fontsize=8, va="top", color=cpal.get(prim, "#555"))

            # P(state) panel
            for lab, g in P.groupby("state_label"):
                ax_st.plot(g["t_s"], g["mean_post"], color=scolor(lab), label=lab, lw=1.6)
                ax_st.fill_between(g["t_s"],
                                   g["mean_post"] - g["sem_post"],
                                   g["mean_post"] + g["sem_post"],
                                   color=scolor(lab), alpha=0.15, lw=0)
            ax_st.axvline(0, color=evt_col, lw=1.4, ls="--")
            if dur:
                ax_st.axvspan(0, dur, color=evt_col, alpha=0.07, lw=0)
            ax_st.set_ylabel("P(state)", fontsize=9)
            ax_st.set_ylim(0, 1)
            ax_st.legend(fontsize=7, frameon=False, ncol=3, loc="upper right")

        axes[-1].set_xlabel("time from event (s)", fontsize=10)
        fig.suptitle(f"Cross-sensor {evlab} — ΔF/F & HMM states (n=1 per sensor, descriptive)",
                     fontsize=11, fontweight="bold", y=1.01)
        plt.tight_layout()
        _save(fig, fd, f"fig_summary_{stem}_hmm", fmts)
